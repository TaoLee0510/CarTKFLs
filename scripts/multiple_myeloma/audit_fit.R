...
rows <- lapply(seq_len(nrow(tasks)), function(i) {
  ...
  outdir <- fit_directory(cfg, task$pm, task$min_obs, task$patient)
  ...
  recorded <- if (!is.null(status) && nrow(status) == 1L) {
    as.character(status$state)
  } else "MISSING_STATUS"
...
if (any(!audit$accepted)) {
  stop(sum(!audit$accepted), " fit tasks have missing or inconsistent status")
}
summary <- do.call(rbind, lapply(names(cfg$patients), function(patient) {
  subset <- audit[audit$patient == patient, , drop = FALSE]
  data.frame(cancer_type = cfg$cancer_type, PatientID = patient,
             complete_raw = sum(subset$recorded_state == "COMPLETE"),
...
usable <- summary[summary$complete_raw > 0L, c("cancer_type", "PatientID", "complete_raw")]
if (!nrow(usable)) stop("No patient has an evaluable ALFA-K fit")
atomic_tsv(usable, file.path(run_dir, "manifests", "kfl_samples.tsv"))
cat(sprintf("Fit audit passed: %d complete; %d model errors; %d no-CV; %d evaluable patients.\n",
            sum(audit$recorded_state == "COMPLETE"),
            sum(audit$recorded_state == "MODEL_ERROR"),
            sum(audit$recorded_state == "NO_CV"), nrow(usable)))
