...
input <- consumer_input(cfg, patient)
outdir <- fit_directory(cfg, pm, minobs, patient)
status_path <- file.path(run_dir, "status", sprintf("fit_%05d.tsv", task_id))

write_status <- function(state, detail = "") {
  detail <- gsub("[\r\n\t]+", " ", as.character(detail))
  atomic_tsv(data.frame(
    task_id = task_id, patient = patient, cancer_type = cfg$cancer_type,
    pm = pm, pm_label = pm_label(pm), min_obs = minobs,
    state = state, detail = detail, job_id = Sys.getenv("SLURM_JOB_ID"),
...
verified <- validate_input(input, patient, cfg)
...
tryCatch({
  ...
  state <<- if (validate_raw(outdir) &&
                grepl("cross-validation|xval|no evaluable", failure,
                      ignore.case = TRUE)) "NO_CV" else "MODEL_ERROR"
})
if (state != "COMPLETE") {
  writeLines(c(
    paste0("task_id=", task_id), paste0("patient=", patient),
    paste0("pm=", pm), paste0("minobs=", minobs),
    paste0("state=", state), paste0("message=", failure)
  ), file.path(outdir, "alfak_failed.txt"))
}
write_status(state, failure)
cat(sprintf("task=%d patient=%s pm=%s minobs=%d state=%s %s\n",
            task_id, patient, pm_label(pm), minobs, state, failure))
